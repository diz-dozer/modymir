...
// shopping cart
if (isset($_REQUEST['shopping_cart'])) {


    $cart_reguest = 'index.php?shopping_cart';
            ...
            if (strstr($key, 'count_')) {
                ...
                if (isset($_SESSION['log'])) //authorized user
                {
                    ...
                    if ($val > 0) //$val is a new items count in the shopping cart
                    {
                        ...
                        SET
                        Quantity=' . floor($val) . '
                        WHERE customerID=' . regGetIdByLogin($_SESSION['log']) . ' AND itemID=' . (int)str_replace('count_', '', $key);
                        $q = db_query($sql);
