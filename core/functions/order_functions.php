                                          ...
                                          $payment_email_comments_text, $shipping_email_comments_text, $tax, $order_active_link)
{
    ...
    $html = $smarty_mail->fetch("customer_order_notification.tpl");

    if (CONF_ACTIVE_ORDER) {

        $html_active = $smarty_mail->fetch("customer_order_activate.tpl");
    ...
    $smarty_mail, $shServiceID = 0)
{
    ...
    $sql = 'INSERT INTO ' . ORDERS_TABLE . ' SET
            ...
            shipping_cost="' . ((float)$shipping_costUC) . '",
            order_discount="' . ((float)$discount_percent) . '",
...
function ordGetOrder($orderID)
{
    ...
    $sql = 'SELECT DISTINCT * FROM ' . ORDERS_TABLE . ' WHERE orderID="' . (int)$orderID . '" LIMIT 1';
