{* zones configuration *}
<form action="{$smarty.const.ADMIN_FILE}?dpt=conf&amp;sub=zones&amp;countryID={$countryID}" method=post enctype="multipart/form-data" name="MainForm" id="MainForm">
    {if $countries}
        <table class="adn">
            <tr>
                <td align="left">
                    <select name='countryID_Select' onchange="window.location=document.getElementById('MainForm').countryID_Select.value">
                ...
                <td align="left" width="100%">&nbsp;&nbsp;&nbsp;<b>{$smarty.const.ADMIN_ZONE_INIT}</b></td>
            </tr>
        </table>
        <table class="adn">
            <tr>
        ...
        <table class="adn">
            ...
            {section name=i loop=$zones}
                <tr class="liney hover">
                    <td>
                        <input type=text name=zone_name_{$zones[i].zoneID} value="{$zones[i].zone_name}" class="textp" size="45">
                    </td>
                    <td>
                        <input type=text name=zone_code_{$zones[i].zoneID} value="{$zones[i].zone_code}" class="textp" size="45">
                    </td>
                    <td align="center">
                        <a href="#" onclick="confirmDelete({$zones[i].zoneID},'{$smarty.const.QUESTION_DELETE_CONFIRMATION}','{$smarty.const.ADMIN_FILE}?dpt=conf&amp;sub=zones&amp;countryID={$countryID}&amp;delete=');">X</a>
                    </td>
                </tr>
                {sectionelse}
                <tr>
        ...
        <table class="adn">
            ...
            <tr class="lins">
                <td><input type="text" name="new_zone_name" class="textp" size="45"></td>
