...
<form name="extform2" method=GET id="extform2" action="">
    {if $customers_has_been_exported_succefully == 1}
        ...
        <div align="left" style="padding-top: 8px;">
            <a href='{$urlToDownloadResult}' class="inl">{$smarty.const.ADMIN_DOWN_LOAD}</a> ( {$file_size} b )
        </div>
        <table class="adn">
            <tr>
                <td class="se6"></td>
    ...
    <table class="adn">
        <tr class="lineb">
            <td align=left width="100%">
                <a href='{$urlToSort}&amp;sort=Login&amp;direction=ASC' class="liv">{$smarty.const.ADMIN_CUSTOMER_LOGIN}</a>
            </td>
            <td align=right>
                <a href='{$urlToSort}&amp;sort=first_name&amp;direction=ASC' class="liv">{$smarty.const.ADMIN_CUSTOMER_FIRST_NAME}</a>
            </td>
            <td align=right>
                <a href='{$urlToSort}&amp;sort=last_name&amp;direction=ASC' class="liv">{$smarty.const.ADMIN_CUSTOMER_LAST_NAME}</a>
            </td>
            <td align=right class="toph3">
                <a href='{$urlToSort}&amp;sort=Email&amp;direction=ASC' class="liv">{$smarty.const.ADMIN_CUSTOMER_EMAIL}</a>
            </td>
            <td align=right>
                <a href='{$urlToSort}&amp;sort=custgroupID&amp;direction=ASC' class="liv">{$smarty.const.ADMIN_CUSTOMER_GROUP_NAME}</a>
            </td>
            <td align=right class="toph3">
                <a href='{$urlToSort}&amp;sort=reg_datetime&amp;direction=ASC' class="liv">{$smarty.const.ADMIN_REGISTRATION_TIME}</a>
            </td>
        ...
        {section name=i loop=$customers}
            {if $admhl eq 1}
                <tr>
                    <td colspan="8" class="separ"><img src="data/admin/pixel.gif" alt="" class="sep"></td>
                </tr>
            {else}{assign var="admhl" value=1}{/if}
            <tr class="lineybig hover">
                <td align="left">
                    <a href='{$smarty.const.ADMIN_FILE}?dpt=custord&amp;sub=custlist&amp;customer_details=contact_info&amp;customerID={$customers[i].customerID}&amp;encodedReturnUrl={$encodedReturnUrl}'>{$customers[i].Login}</a>
                </td>
                ...
                <td align="left" class="toph3">{if $customers[i].ActivationCode}{$smarty.const.STR_NOTACTIVATED}
                    <br><a href="{"activateID=`$customers[i].customerID`"|set_query}" onclick="return confirm('{$smarty.const.STR_QACTIVATE_ACCOUNT|replace:'[email]':$customers[i].Email}');">{$smarty.const.ACTIVATE_BUTTON}</a>{else}{$smarty.const.STR_ACTIVATED}{/if}
                </td>
                <td align="center">{if $customers[i].allowToDelete}
                    <a href="#" onclick="confirmDelete({$customers[i].customerID}, '{$smarty.const.QUESTION_DELETE_CONFIRMATION}', '{$smarty.const.ADMIN_FILE}?dpt=custord&amp;sub=custlist&amp;encodedReturnUrl={$encodedReturnUrl}&amp;deleteCustomerID=')">X</a>{/if}
                </td>
            </tr>
            {sectionelse}
            <tr>
        <td colspan="8" nowrap align="center" height="22">{$smarty.const.ADMIN_NO_VALUES}</td></tr>
        {/section}
        {if $navigator}
        <tr>
    ...
    <input type=hidden name='export_to_excel' value='1'> <input type=hidden name='dpt' value='custord'>
    ...
    <a href="#" onclick="document.getElementById('extform2').submit(); return false" class="inl">{$smarty.const.ADMIN_EXPORT_USER_TO_FILE}</a>{/if}
{else}
{if $customerID eq NULL}
{$smarty.const.ADMIN_NO_RECORD_FOUND}
{if $returnUrl}<a href='{$returnUrl}'>
    {else}
    ...
    <table width="100%" border="0" class="usert" cellspacing="0">
        ...
        <tr valign="top" class="lineb">
            ...
            {else}
                <td align="left" width="20%" style="background: #E7ECFF; padding: 10px; border: 1px solid #80A2D9;" valign="middle">
                    <a href="{$smarty.const.ADMIN_FILE}?dpt=custord&amp;sub=custlist&amp;customer_details=contact_info&amp;customerID={$customerID}&amp;encodedReturnUrl={$encodedReturnUrl}" class="liv">{$smarty.const.STRING_CONTACT_INFORMATION_DOWN_CASE}</a>
                </td>
            {/if}
            ...
            {else}
                <td align="left" width="20%" style="background: #E7ECFF; padding: 10px; border: 1px solid #80A2D9; border-left: 0px;" valign="middle">
                    <a href="{$smarty.const.ADMIN_FILE}?dpt=custord&amp;sub=custlist&amp;customer_details=address_book&amp;customerID={$customerID}&amp;encodedReturnUrl={$encodedReturnUrl}" class="liv">{$smarty.const.STRING_ADDRESS_BOOK}</a>
                </td>
            {/if}
            ...
            {else}
                <td align="left" width="20%" style="background: #E7ECFF; padding: 10px; border: 1px solid #80A2D9; border-left: 0px;" valign="middle">
                    <a href="{$smarty.const.ADMIN_FILE}?dpt=custord&amp;sub=custlist&amp;customer_details=order_history&amp;customerID={$customerID}&amp;encodedReturnUrl={$encodedReturnUrl}" class="liv">{$smarty.const.STRING_ORDER_HISTORY}</a>
                </td>
            {/if}
            ...
            {else}
                <td align="left" width="20%" style="background: #E7ECFF; padding: 10px; border: 1px solid #80A2D9; border-left: 0px;" valign="middle">
                    <a href="{$smarty.const.ADMIN_FILE}?dpt=custord&amp;sub=custlist&amp;customer_details=visit_log&amp;customerID={$customerID}&amp;encodedReturnUrl={$encodedReturnUrl}" class="liv">{$smarty.const.STRING_VISIT_HISTORY}</a>
                </td>
            {/if}
            ...
            {else}
                <td align="left" width="20%" style="background: #E7ECFF; padding: 10px; border: 1px solid #80A2D9; border-left: 0px;" valign="middle">
                    <a href="{$smarty.const.ADMIN_FILE}?dpt=custord&sub=custlist&customer_details=affiliate&customerID={$customerID}&encodedReturnUrl={$encodedReturnUrl}" class="liv">{$smarty.const.STRING_AFFILIATE_PROGRAM}</a>
                </td>
            {/if}
        </tr>
    </table>
                        ...
                        <table class="adw">
                            ...
                            {if $customerInfo.allowToDelete}
                                <tr class="liney">
                                    <td></td>
                                    <td>
                                        <input type="button" onclick="if(window.confirm('{$smarty.const.QUESTION_DELETE_CONFIRMATION}'))document.location.href='{"&safemode=&deleteCustomerID=`$customerInfo.customerID`"|set_query}';" value="{$smarty.const.TERMINATE_ACCOUNT_LINK}" style="font-size: 11px; font-family: Tahoma, Arial; border: 1px solid #80A2D9; background-color: #E1ECFD;">
                                    </td>
                                </tr>
                            {/if}
