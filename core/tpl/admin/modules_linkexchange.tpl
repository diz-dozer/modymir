<form action="" method="POST" name="form_delete_linkcategory" id="form_delete_linkcategory">
    ...
    <input name="fREDIRECT" value="{$REQUEST_URI}" type="hidden"> <input name="LINK_CATEGORY[le_cID]" type="hidden">
</form>{section name=i loop=$le_categories}{if $le_categories[i].le_cID==$le_CategoryID}{assign var='le_CategoryName' value=$le_categories[i].le_cName}{/if}{/section}
<div class="edit_le_category" id="category_renblock"></div>
<table class="adw">
    <tr>
        <td align="left" valign="top">
            <span class="titlecol2">{if $le_CategoryName && $le_CategoryName != ""}{$le_CategoryName}{else}{$smarty.const.ADMIN_LE_ALL_CATEGORIES}{/if}</span>
        </td>
    </tr>
</table>
<table class="adn">
    ...
    <div id="le_links">
        <form action="" method="POST" id="form_change_links">
            ...
            <table class="adn">
                <tr class="lineb">
                    <td align="center">
                        <input id="id_checkall" onclick="checkBoxes('form_change_links', 'id_checkall', 'id_ch');" type="checkbox" class="round">
                    </td>
                ...
                {foreach from=$le_links item=_le_link}
                    ...
                    <tr class="liney">
                        <td align="center">
                            <input name="LINKS_IDS[]" value="{$_le_link.le_lID}" type="checkbox" id="id_ch" class="round">
                        </td>
                        <td align="left">
                            <input name="LINK[{$_le_link.le_lID}][le_lURL]" value="{$_le_link.le_lURL}" type="text" class="textp" size="24">
                        </td>
                        <td align="left">
                            <input name="LINK[{$_le_link.le_lID}][le_lText]" size="24" value="{$_le_link.le_lText}" type="text" class="textp">
                        </td>
                        <td align="left">
                            <input name="LINK[{$_le_link.le_lID}][le_lDesk]" size="24" value="{$_le_link.le_lDesk}" type="text" class="textp">
                        </td>
                ...
                <tr>
                    <td colspan="5" class="navigator">{if $curr_page > 1}
                        <a href="{"page=`$curr_page-1`&amp;show_all="|set_query}">{$smarty.const.STRING_PREVIOUS}</a>&nbsp;|{/if}{foreach from=$le_lister_range item=_page}&nbsp;{if $_page!=$curr_page or $showAllLinks}
                            <a class="no_underline" href="{"page=`$_page`&amp;show_all="|set_query}">{$_page}</a>{else}
                            <b>{$_page}</b>{/if}&nbsp;|{/foreach}
                        {if $curr_page < $last_page}
                            <a href="{"page=`$curr_page+1`&amp;show_all="|set_query}">{$smarty.const.STRING_NEXT}</a>&nbsp;|&nbsp;{/if}{if $showAllLinks}<b>{$smarty.const.STRING_SHOWALL}</b>{else}
                        <a class="no_underline" href="{"show_all=yes"|set_query}">{$smarty.const.STRING_SHOWALL}</a>{/if}
                    </td>
                </tr>
            </table>
            <table class="adn">
            ...
            <table class="adw">
                <tr>
                    <td>{$smarty.const.STRING_LE_TODO} <select name="new_le_lCategoryID">
...
{else}
    <table class="adn">
        ...
        <tr>
            <td align="center" valign="middle" height="24">{$smarty.const.ADMIN_NO_LINKS_IN}</td>
        </tr>
    </table>
    <table class="adn">
        <tr>
...
<div class="new_link_category" id="id_new_linkcategory">
    <form action="" method="POST" id="form_new_linkcategory" name="form_new_linkcategory">
        ...
        <table class="adn">
            ...
            <tr class="lins">
                <td><input name="LINK_CATEGORY[le_cName]" type="text" class="textp" size="36" id="id_new_category"></td>
            </tr>
        </table>
        <table class="adn">
            <tr>
...
<div id="le_new_link">
    <form action="" id="form_new_le_link" method="POST" name="form_new_le_link">
        ...
        <table class="adn">
            <tr class="lineb">
                <td align="left">{$smarty.const.ADMIN_LE_NEW_CATEGORY_NAME_LINK}</td>
            </tr>
            <tr class="lins">
                <td align="left">{$smarty.const.ADMIN_LE_LINK_CATEGORY}: <select name="LINK[le_lCategoryID]">
                    ...
                    <input name="LINK[le_lText]" value="{$pst_LINK.le_lText}" type="text" size="44" class="textp">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{$smarty.const.STRING_DESCRIPTION}:
                    <input name="LINK[le_lDesk]" value="{$pst_LINK.le_lDesk}" type="text" size="44" class="textp"></td>
            </tr>
        </table>
    </form>
</div>
...
<table class="adn">
    <tr>
        <td class="help"><span class="titlecol2">{$smarty.const.USEFUL_FOR_YOU}</span>
            ...
            <div class="helptext">{$smarty.const.ALERT_ADMIN_LINKEDIT}</div>
        </td>
    </tr>
</table>{if $show_new_link}
    {if $last_page}
    ...
    {/if}
{/if}{if $error_message}
<script type="text/javascript" defer>alert('{$error_message}')</script>{/if}