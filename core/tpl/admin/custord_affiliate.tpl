...
{if $smarty.const.CONF_AFFILIATE_PROGRAM_ENABLED}
    ...
    {if $show_tables}
        ...
        {if $CommissionsNumber}
            {if $delete_commission}
                <div style="color: green;">{$smarty.const.AFFP_MSG_COMMISSION_DELETED}</div>
                <br>
            {/if}
            ...
            <table class="adn">
                ...
                <tr class="lineb">
                    <td align=left>
                        <a href="{'OrderFieldC=cID&OrderDivC=DESC'|set_query:$REQUEST_URI}" class="liv">ID</a></td>
                    <td align=left>
                        <a href="{'OrderFieldC=xDateTime&OrderDivC=DESC'|set_query:$REQUEST_URI}" class="liv">{$smarty.const.ADMIN_CURRENT_DATE}</a>
                    </td>
                    <td align=left>{$smarty.const.TABLE_CUSTOMER}</td>
                    <td align=left width="80%">
                        <a href="{'OrderFieldC=Description&OrderDivC=ASC'|set_query:$REQUEST_URI}" class="liv">{$smarty.const.STRING_DESCRIPTION}</a>
                    </td>
                    <td align=left>
                        <a href="{'OrderFieldC=Amount&OrderDivC=ASC'|set_query:$REQUEST_URI}" class="liv">{$smarty.const.STRING_SUM}</a>
                    </td>
                    <td align=left>&nbsp;</td>
                ...
                {foreach from=$Commissions item=_Commission}
                    {if $admhl eq 1}
                        <tr>
                            <td colspan="6" class="separ"><img src="data/admin/pixel.gif" alt="" class="sep"></td>
                        </tr>
                    ...
                    <td align=left class="toph3">
                        <a href="{''|set_query}?dpt=custord&sub=custlist&customer_details=contact_info&customerID={$_Commission.customerID}" class="inl">{$_Commission.CustomerLogin}</a>
                    </td>
                    <td align=left>{$_Commission.Description}</td>
                    <td align=left class="toph3">{$_Commission.Amount} {$_Commission.CurrencyISO3}</td>
                    <td align=left>
                        <a href="javascript:open_window('{"sub_page=edit_commission&cID=`$_Commission.cID`"|set_query:$REQUEST_URI}',500,400);">{$smarty.const.EDIT_BUTTON}</a>&nbsp;|&nbsp;<a href="javascript:void(0)" onclick="if(!confirm('{$smarty.const.STRING_LE_CONFIRM}'))return false;document.form_delete_commission.elements['COMMISSION[cID]'].value = {$_Commission.cID};document.form_delete_commission.submit();return false;" title="{$smarty.const.DELETE_BUTTON}">X</a>
                    </td>
                    </tr>{/foreach}
                <tr>
                    <td class="navigator" colspan="6">
                        <table width="100%">


                            <tr>
                                <td alihn=right width="100%" class="toph3">{$smarty.const.STRING_SUM}:&nbsp;</td>
                                <td align="right">
                                    <table align="right">
                                        {foreach from=$TotalCommissionsAmount key=_key item=_total}
                                            <tr style="font-weight: bold">
                                                <td align="right" class="toph3">{$_total|string_format:"%.2f"}</td>
                                                <td class="toph3">{$_key}</td>
                                            </tr>
                                        {/foreach}
                                    </table>
                                </td>
        ...
        {if $PaymentsNumber}
            {if $delete_payment}
                <div style="color: green;">{$smarty.const.AFFP_MSG_PAYMENT_DELETED}</div>
                <br>
            {/if}
            ...
            <table class="adn">
                ...
                <tr class="lineb">
                    <td align=left><a href="{'OrderField=pID&OrderDiv=DESC'|set_query:$REQUEST_URI}" class="liv">ID</a>
                    </td>
                    <td align=left>
                        <a href="{'OrderField=xDate&OrderDiv=DESC'|set_query:$REQUEST_URI}" class="liv">{$smarty.const.ADMIN_CURRENT_DATE}</a>
                    </td>
                    <td align=left>{$smarty.const.TABLE_CUSTOMER}</td>
                    <td align=left width="80%">
                        <a href="{'OrderField=Description&OrderDiv=ASC'|set_query:$REQUEST_URI}" class="liv">{$smarty.const.STRING_DESCRIPTION}</a>
                    </td>
                    <td align=left>
                        <a href="{'OrderField=Amount&OrderDiv=ASC'|set_query:$REQUEST_URI}" class="liv">{$smarty.const.STRING_SUM}</a>
                    </td>
                    <td align=left>&nbsp;</td>
                </tr>{assign var="admhl" value=0}
                {foreach from=$Payments item=_Payment}
                    ...
                    <td align=left class="toph3">
                        <a href="{''|set_query}?dpt=custord&sub=custlist&customer_details=contact_info&customerID={$_Payment.customerID}" class="inl">{$_Payment.CustomerLogin}</a>
                    </td>
                    <td align=left>{$_Payment.Description}</td>
                    <td align=left class="toph3">{$_Payment.Amount} {$_Payment.CurrencyISO3}</td>
                    <td align=left>
                        <a href="javascript:open_window('{"sub_page=edit_payment&pID=`$_Payment.pID`"|set_query:$REQUEST_URI}',500,400);">{$smarty.const.EDIT_BUTTON}</a>&nbsp;|&nbsp;<a href="javascript:void(0)" onclick="if(!confirm('{$smarty.const.STRING_LE_CONFIRM}'))return false;document.form_delete_payment.elements['PAYMENT[pID]'].value = {$_Payment.pID};document.form_delete_payment.submit();return false;" title="{$smarty.const.DELETE_BUTTON}">X</a>
                    </td>
                    </tr>{/foreach}
                <tr>
                    <td class="navigator" colspan="6">
    ...
    {if $error_new_commission}
        <div style="color: red">{$error_new_commission}</div>
        <br>
    {/if}
    {if $newCommissionStatus}
        <div style="color: green">{$smarty.const.AFFP_MSG_NEW_COMMISSION_OK}</div>
        <br>
    {/if}
    <form method="POST" action="{'#new_commission_anchor'|set_query:$REQUEST_URI}" id="newcommform">
        ...
        <table class="adn">
            ...
            <tr class="lins">
                <td align=left>
                    <input name="NEW_COMMISSION[xDate]" value="{$NEW_COMMISSION.xDate}" type="text" size="20" class="prc">
                </td>
                <td align=left>
                    <input name="NEW_COMMISSION[Amount]" value="{$NEW_COMMISSION.Amount}" type="text" size="20" class="prc">&nbsp;
                ...
                <td align=left>
                    <input name="NEW_COMMISSION[customerLogin]" class="prc" value="{if $NEW_COMMISSION.customerLogin}{$NEW_COMMISSION.customerLogin}{elseif $edCustomerLogin}{$edCustomerLogin}{/if}" type="text" size="20">
                </td>
                <td align=left>
                    <textarea name="NEW_COMMISSION[Description]" style="height: 44px; width: 170px;">{$NEW_COMMISSION.Description}</textarea>
                </td>
            </tr>
        </table>
    </form>
    ...
    {if $error_new_payment}
        <div style="color: red">{$error_new_payment}</div>
        <br>
    {/if}
    {if $newPayStatus}
        <div style="color: green">{$smarty.const.AFFP_MSG_NEW_PAY_OK}</div>
        <br>
    {/if}
    <form method="POST" action="{'#new_payment_anchor'|set_query:$REQUEST_URI}" id="newpayyform">
        ...
        <table class="adn">
            ...
            <tr class="lins">
                <td align=left>
                    <input name="NEW_PAYMENT[xDate]" value="{$NEW_PAYMENT.xDate}" type="text" size="20" class="prc">
                </td>
                <td align=left>
                    <input name="NEW_PAYMENT[Amount]" value="{$NEW_PAYMENT.Amount}" type="text" size="20" class="prc">&nbsp;
                ...
                <td align=left>
                    <input name="NEW_PAYMENT[customerID]" class="prc" value="{if $NEW_PAYMENT.customerID}{$NEW_PAYMENT.customerID}{elseif $edCustomerLogin}{$edCustomerLogin}{/if}" type="text" size="20">
                </td>
                <td align=left>
                    <textarea name="NEW_PAYMENT[Description]" style="height: 44px; width: 170px;">{$NEW_PAYMENT.Description}</textarea>
                </td>
            </tr>
        </table>
    </form>
    ...
    <table class="adn">
        <tr>
            <td class="help"><span class="titlecol2">{$smarty.const.USEFUL_FOR_YOU}</span>
                ...
                <div class="helptext">{$smarty.const.ALERT_ADMIN2}</div>
            </td>
        </tr>
    </table>
{else}
