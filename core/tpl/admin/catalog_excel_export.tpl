{if $excel_export_successful eq 1}
    ...
    <a href="{$smarty.const.ADMIN_FILE}?do=get_file&amp;getFileParam={$getFileParam}" class="inl">{$smarty.const.ADMIN_DOWN_LOAD}</a>
    ({$excel_filesize} МБ)
{else}
    ...
    <form action="{$smarty.const.ADMIN_FILE}" method=post name="form" id="form">
        ...
        <input type="hidden" name="dpt" value="catalog"> <input type="hidden" name="sub" value="excel_export">
...
<table class="adn">
    <tr>
        <td class="help"><span class="titlecol2">{$smarty.const.USEFUL_FOR_YOU}</span>
            ...
            <div class="helptext">{$smarty.const.STRING_EXCEL_EXPORT_DESCRIPTION}</div>
        </td>
    </tr>
</table>