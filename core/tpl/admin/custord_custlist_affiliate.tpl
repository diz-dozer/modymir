{if $smarty.const.CONF_AFFILIATE_PROGRAM_ENABLED}
    ...
    {if $safemode}
        <div style="color: red;">{$smarty.const.ADMIN_SAFEMODE_WARNING}</div>
        <br>
    {/if}
    {if $smarty.const.CONF_USER_SYSTEM eq 2}{$smarty.const.STRING_AFFILIATE_CUSTOMERS}: {$RecruitedCustomersNumber}<br>
        <br>
    {/if}
            ...
            {foreach from=$RecruitedCustomers item=_Customer name=i}
                ...
                <tr class="liney hover">
                    ...
                    <td align=right width="1%">
                        <a href="javascript:void(0)" onclick="if(!confirm('{$smarty.const.STRING_LE_CONFIRM}'))return false;document.form_cancel_customer.elements['CUSTOMER[customerID]'].value = {$_Customer.customerID};document.form_cancel_customer.submit();return false;" title="{$smarty.const.AFFP_REMOVE_USER}">X</a>
                    </td>
                </tr>
            {/foreach}
    ...
    <a href="?dpt=custord&amp;sub=affiliate&amp;edCustomerID={$edCustomerID}#new_commission_anchor" class="inl">{$smarty.const.AFFP_SUBMIT_NEW_COMMISSION}</a>
    &nbsp;&nbsp;|&nbsp;&nbsp;
    <a href="?dpt=custord&amp;sub=affiliate&amp;edCustomerID={$edCustomerID}#new_payment_anchor" class="inl">{$smarty.const.AFFP_SUBMIT_NEW_PAYMENT}</a>
    <br>
    <br>
    ...
    <form method="POST" action="{$REQUEST_URI}">
        {if $Error_DateFormat}
            <div style="color: red;">{$smarty.const.AFFP_MSG_ERROR_DATE_FORMAT}</div>
            <br>
        {/if}
        {$smarty.const.STRING_FROM}:
        <input type="text" name="from" value="{$from}" size="12"> {$smarty.const.STRING_TILL}:
        <input name="till" value="{$till}" type="text" size="12"/>
        <input value="{$smarty.const.VIEW_BUTTON}" type="submit" style="font-size: 11px; font-family: Tahoma, Arial; border: 1px solid #80A2D9; background-color: #E1ECFD;">
    </form>
    {if !$show_tables}
        <br>
    ...
    {if $CommissionsNumber and $show_tables}
        ...
        <table class="adn">
            <tr class="lineb">
                <td align=left><a href="{'OrderFieldC=cID&OrderDivC=DESC'|set_query:$REQUEST_URI}" class="liv">ID</a>
                </td>
                <td align=left>
            ...
            {foreach from=$Commissions item=_Commission}
                {if $admhl eq 1}
                    <tr>
                        <td colspan="5" class="separ"><img src="data/admin/pixel.gif" alt="" class="sep"></td>
                    </tr>
                {else}{assign var="admhl" value=1}{/if}
    ...
    {elseif $show_tables}
        <div align="left" style="padding: 4px 6px;">{$smarty.const.AFFP_MSG_NOCOMMISISONS_FOUND}</div>
        <table class="adn">
            <tr>
                <td class="separ"><img src="data/admin/pixel.gif" alt="" class="sep"></td>
            </tr>
    ...
    {elseif $show_tables}
        <div align="left" style="padding: 4px 6px;">{$smarty.const.AFFP_MSG_NOPAYMENTS_FOUND}</div>
        <table class="adn">
            <tr>
                <td class="separ"><img src="data/admin/pixel.gif" alt="" class="sep"></td>
            </tr>
