...
{/literal}<input name="expandID" type="hidden"><input name="unexpandID" type="hidden">
<input name="showProducts" type="hidden"><input name="updateCategory" value="" type="hidden">
...
<tr>
    <td style="padding-left: {$_ProductCategory.level*20}px" valign="top">
        {if $_ProductCategory.ExistSubCategories}
            {if $_ProductCategory.ExpandedCategory}
                <a href="#" onclick="document.form_export.elements['unexpandID'].value = {$_ProductCategory.categoryID};document.form_export.submit();return false;"><img src="data/admin/minus.gif" alt=""></a>
            {else}
                <a href="#" onclick="document.form_export.elements['expandID'].value = {$_ProductCategory.categoryID};document.form_export.submit();return false;">
                <img src="data/admin/mplus.gif" alt=""></a>
            {/if}{else}<img src="data/admin/dr.gif" alt="">{/if}</td>
    <td valign="top">
        {count array=$smarty.session.selectedProducts[$_ProductCategory.categoryID] item=_ProductsNum}
        <input name="CHECKED_CATEGORIES[{$_ProductCategory.categoryID}]" type="checkbox" class="round" value="1" onclick="document.form_export.elements['updateCategory'].value = {$_ProductCategory.categoryID};document.form_export.submit();return false;" id="{$_ProductCategory.categoryID}"
    ...
    <td width="100%" style="padding: 0 0 4px 4px;">
        {if $_ProductCategory.products_count_category}
            <a class="inl" href="#" onclick="document.form_export.elements['showProducts'].value = {$_ProductCategory.categoryID};document.form_export.submit();return false;">{$_ProductCategory.name}</a>
        {else}
        ...
        {if $showProducts==$_ProductCategory.categoryID and $ProductsNum}
            ...
            <table class="adn">
                <tr class="lineb">
                    <td align="center">
                        <input type="checkbox" onclick="checkAllProducts({$ProductsNum})" id="id_checkallprod"
                ...
                {foreach from=$Products item=_Product}
                    ...
                    <tr class="liney">
                        <td align="center">
                            <input class="round" name="PRODUCTS[{$_Product.productID}]" value="1" id="prod{counter}" type="checkbox"  {if $smarty.session.selectedProducts[$showProducts][$_Product.productID]} checked="checked"  {/if}>
                        </td>
            ...
            <input name="save_products" value="1" type="hidden">
            <a href="#" onclick="document.getElementById('form_export').submit(); return false;" class="inl">{$smarty.const.SELECT_BUTTON}</a>
            <table class="adn">
                <tr>
                    <td class="se6"></td>
                </tr>
