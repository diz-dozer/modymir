{* Шаблон вывода заказа в отдельном окне на печать *}<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
...
{else}
                    ...
                    {if $orderContent[i].eproduct_filename}
                        {if $completed_order_status}
                            {if $completed_order_status == $order.statusID}
                                <br>
                                <a href='{$smarty.const.ADMIN_FILE}?do=get_file&amp;getFileParam={$orderContent[i].getFileParam}' class="sin">{$smarty.const.ADMIN_DOWN_LOAD} ({$orderContent[i].file_size} MB)</a>
    ...
    <table class="adn">
        <tr class="liney">
            <td width="99%" align="right">{$smarty.const.STRING_PRED_TOTAL}:</td>
